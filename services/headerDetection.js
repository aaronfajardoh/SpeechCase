 ...
 * @returns {number} Count of capitalized words
 */
 ...
 * @returns {Object} { isHeader: boolean, confidence: number, signals: Object }
 */
export function detectHeader(text, followingText = '') {
  ...
  if (followingText && followingText.trim().length > 0) {
    const firstChar = followingText.trim()[0];
...
/**
 * Detect headers in a text and return positions
 * @param {string} text - Full text to analyze
 * @param {number} segmentLength - Length of segments to check (default: first 200 chars of each potential header)
 * @returns {Array<Object>} Array of { start: number, end: number, isHeader: boolean, confidence: number }
 */
export function detectHeadersInText(text, segmentLength = 200) {
  if (!text || typeof text !== 'string') {
    return [];
  }

  const results = [];
  // Split by sentences or line breaks to get potential header segments
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // Check first part of segment (up to segmentLength chars)
    const segmentToCheck = segment.substring(0, segmentLength);
